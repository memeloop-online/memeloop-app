...
export class EndToEndArgumentsValidationError extends Error {
  constructor(
    public readonly title: string,
    public readonly lines: string[],
  ) {
    super(title);
    this.name = 'EndToEndArgumentsValidationError';
  }
}
...
function validateTagValue(tagValue: string): void {
  if (tagValue.length > 0 && !tagValue.startsWith('@') && !tagValue.startsWith('not ')) {
    throw new EndToEndArgumentsValidationError(
...
export function formatEndToEndArgumentsValidationError(error: EndToEndArgumentsValidationError): string {
...
function ensureCalibrationFileExists(): void {
  ...
  console.error(formatEndToEndArgumentsValidationError(
    new EndToEndArgumentsValidationError(
...
function isDirectExecution(): boolean {
  ...
  return entry.replace(/\\/g, '/').endsWith('scripts/runEndToEnd.ts');
}
...
if (isDirectExecution()) {
  ...
  try {
    ...
    if (error_ instanceof EndToEndArgumentsValidationError) {
      console.error(formatEndToEndArgumentsValidationError(error_));
      process.exit(1);
    }
    throw error_;
  }
