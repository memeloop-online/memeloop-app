...
Testing guide for memeloop-desktop using Vitest + React Testing Library for unit tests and Playwright + Cucumber for E2E tests.
...
# Timing: ~2-5 minutes on first run (builds and packages the app). Subsequent runs with no src changes are faster.
...
# Timing: ~1-3 minutes depending on test suite size and machine speed.
pnpm test:e2e
...
test-data/           # containing test data folders created during `test:e2e`
dev-data/           # containing dev data folders created during `start:dev`
```
