...
export const EditAgentDefinitionContent: React.FC<EditAgentDefinitionContentProps> = ({ tab }) => {
  ...
  const handleCreatePersistentConversation = useCallback(async () => {
    if (!agentDefinition || isCreatingConversation) return;
    ...
    try {
      ...
      void window.service.native.log('error', 'Failed to create persistent agent conversation for scheduling', {
        error,
        agentDefId: agentDefinition.id,
      });
    } finally {
  ...
  }, [addTab, agentDefinition, isCreatingConversation, saveToBackendDebounced]);
  ...
  return (
    <Container_>
      <ScrollableContent>
        ...
        <Alert
          severity='info'
          data-testid='edit-agent-schedule-persistent-notice'
          action={
            <Button
              color='inherit'
              size='small'
              disabled={isCreatingConversation}
              onClick={() => void handleCreatePersistentConversation()}
            >
              {isCreatingConversation
                ? t('EditAgent.ScheduleCreatingConversation')
                : t('EditAgent.ScheduleCreateConversation')}
            </Button>
          }
        >
          <Typography variant='subtitle2'>{t('EditAgent.SchedulePersistentTitle')}</Typography>
          <Typography variant='body2'>{t('EditAgent.SchedulePersistentDescription')}</Typography>
        </Alert>
