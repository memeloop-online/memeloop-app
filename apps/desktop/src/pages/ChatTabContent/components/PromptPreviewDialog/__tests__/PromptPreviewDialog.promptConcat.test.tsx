...
describe('PromptPreviewDialog - Tool Information Rendering', () => {
  ...
  it('should not inject tool info for disabled plugins', async () => {
    ...
    expect(allPromptsText).not.toContain('wiki-search');
    // Verify other tools are still present
    expect(allPromptsText).toContain('git-log');
  });
  ...
  it('should render workspaces and tools info from real concatPrompt execution', async () => {
    ...
    // Wiki tools removed in memeloop-desktop, verify other tools are present
    // Check for git tool insertion
    let gitElement: IPrompt | undefined = childrenAfterBeforeTool.find((c: IPrompt) => {
      const body = `${c.caption ?? ''} ${c.text ?? ''}`;
      return /git-log/i.test(body);
    });
    if (!gitElement) {
      gitElement = findPromptNodeByText(result?.processedPrompts, /git-log/i);
    }
    expect(gitElement).toBeDefined();

    // Verify the order: before-tool -> plugin content -> post-tool
