...
const mockGitServerService: IGitServerService = {
  ...
  runGitCommand: vi.fn(),
  writeTempGitFile: vi.fn(),
  deleteTempGitFile: vi.fn(),
};
...
// Store the git handler created directly from createGitHandler
...
describe('MemeloopNode git handler', () => {
  ...
  beforeEach(async () => {
    ...
    capturedGitHandler = (service as any).createGitHandler();
