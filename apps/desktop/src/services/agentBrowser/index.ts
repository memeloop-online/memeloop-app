...
export class AgentBrowserService implements IAgentBrowserService {
  ...
  /**
   * Find existing or create new "Talk with AI" tab
   * Only reuses the currently active tab if it's a split view with matching chat child.
   * Otherwise creates a new chat tab.
   */
  public async findOrCreateTalkWithAITab(
    ...
    try {
      ...
      if (activeTabId) {
        ...
        // Check if the active tab is a split view with a chat child
      ...
      // No matching active tab found, create new Chat tab
      const agentInstanceService = container.get<IAgentInstanceService>(serviceIdentifier.AgentInstance);
