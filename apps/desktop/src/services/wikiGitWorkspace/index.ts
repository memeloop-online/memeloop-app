...
/**
 * A workspace owns a Git repository only when its root contains a .git
 * directory/file. Git discovery that walks parent directories would
 * accidentally attach a wiki nested inside another checkout to that parent.
 */
export async function hasOwnGitRepository(workspacePath: string): Promise<boolean> {
  return await pathExists(path.join(workspacePath, '.git'));
}
...
export class WikiGitWorkspace implements IWikiGitWorkspaceService {
  ...
  public initWikiGitTransaction = async (newWorkspaceConfig: INewWikiWorkspaceConfig, userInfo?: IGitUserInfos): Promise<IWorkspace | undefined> => {
    ...
    try {
      ...
      if (await hasOwnGitRepository(wikiFolderLocation)) {
