...
describe('Message Management Plugin - Real Database Integration', () => {
  ...
  describe('Real Wiki Search Scenario - The Missing Tool Result Bug', () => {
    it.skip('should persist all messages in wiki search flow: user query → AI tool call → tool result → AI final response', async () => {
...
该条目创建于2024年，是导航整个wiki内容的重要入口页面。
    ...
    it.skip('should handle multiple tool results in one execution', async () => {
      const agentFrameworkContext = createAgentFrameworkContext();
    ...
    it.skip('should maintain message integrity when reloading from database (simulating page refresh)', async () => {
