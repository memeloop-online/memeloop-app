...
function getElectronApp(): Pick<Electron.App, 'getPath' | 'setPath'> {
  ...
  const userDataFallback = path.resolve(process.cwd(), 'userData-test');
  return {
    getPath: (name: string) => {
      if (name === 'userData') return userDataFallback;
      return path.join(userDataFallback, `electron-fallback-${name}`);
    },
    setPath: () => {},
  };
}
